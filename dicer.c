...
void print_results_to_csv(FILE *f,char *time, const pid_t pid, int core, struct pqos_event_values *data) {
	double ipc_o = data->ipc;
	...
	for (j=0; j<NUM; j++){
		if ((p = waitpid(script_table[j], &status, WNOHANG)) == -1){
			...
			if (WIFEXITED(status)){
				...
				const struct pqos_event_values *pointer = &inst_data[j]->values;
				info_table[j]->cycles = info_table[j]->cycles + (uint64_t) pointer->ipc_unhalted;
				info_table[j]->instructions = info_table[j]->instructions + (uint64_t) pointer->ipc_retired;
